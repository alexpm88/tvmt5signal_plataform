        ...
        <Card className="border-0 bg-transparent shadow-none">
          <CardHeader className="relative z-10">
            <div className="flex items-center justify-between">
              <CardTitle className="text-xl font-semibold text-white hover:text-cyan-900 transition-colors duration-200">Recent Signals</CardTitle>
          ...
          <CardContent>
            <div className="space-y-3">
              {signals.length === 0 ? (
                <div className="text-center py-8 text-white/70 hover:text-black/70 transition-colors duration-200">No recent signals found</div>
              ) : (
                signals.map((signal, index) => (
                  <motion.div
                    ...
                    className="flex items-center justify-between p-4 rounded-lg bg-white/10 hover:bg-white/90 transition-colors duration-200 backdrop-blur-sm group"
                  >
                    <div className="flex items-center space-x-4">
                      <div className="flex flex-col">
                        <div className="flex items-center space-x-2">
                          <span className="font-semibold text-white group-hover:text-black">{signal.symbol}</span>
                          <Badge
                        ...
                        <span className="text-sm text-white/70 group-hover:text-black/70">
                          {mounted 
                    ...
                    <div className="flex items-center space-x-4">
                      {signal.entry_price && (
                        <div className="text-right">
                          <div className="text-sm text-white/70 group-hover:text-black/70">Entry</div>
                          <div className="font-medium text-white group-hover:text-black">{signal.entry_price.toFixed(5)}</div>
                        </div>
                      )}

                      {signal.exit_price && (
                        <div className="text-right">
                          <div className="text-sm text-white/70 group-hover:text-black/70">Exit</div>
                          <div className="font-medium text-white group-hover:text-black">{signal.exit_price.toFixed(5)}</div>
                        </div>
                      )}

                      {signal.pnl !== null && signal.pnl !== undefined && (
                        <div className="text-right">
                          <div className="text-sm text-white/70 group-hover:text-black/70">P&L</div>
